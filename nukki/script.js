...
let rawAlpha = null;   
let bgRaw = 0, bgGRaw = 0, bgBRaw = 0, bgDarkRaw = false; 
...
function preprocessImage(imageData) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        tensor[i + n]     = (data[i * 4 + 1] / 255 - MEAN[1]) / STD[1]; 
        tensor[i + 2 * n] = (data[i * 4 + 2] / 255 - MEAN[2]) / STD[2]; 
    }
    return tensor;
}
...
window.addEventListener('mouseup', () => {
    if (isBrushPainting) _brushCache = null; 
